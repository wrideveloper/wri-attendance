...
@section('content')
{{-- @include('components.konfirmasi', ['text' => 'Apakah Anda yakin ingin memperbarui profil?']) ga tau kenapa, kalau pakai ini malah ga bisa --}}
<div class="container pb-5 px-4 d-flex flex-column justify-content-center">
    <h5 class="mb-4 mb-md-5">Edit Profile</h5>
    <div class="col-md-12">
        <form class="edit-profil form rounded bg-light py-4 px-5 px-md-5 bg-white py-0 py-md-2" method="POST" action="/user/{{ $user->nim }}">
            @method('put')
            @csrf
            <div class="form-group mb-3">
                <label for="name" class="form-label fs-5">Nama</label>
                <input name="name" id="name" type="text" class="form-control" placeholder="Input your name" value="{{ old('name', $user->name) }}" required>
            </div>
            <div class="form-group mb-3">
                <label for="email" class="form-label fs-5">Email</label>
                <input name="email" id="email" type="text" class="form-control" placeholder="Input your email" value="{{ old('name', $user->email) }}" required>
            </div>
            <div class="col-12">
                <div class="row">
